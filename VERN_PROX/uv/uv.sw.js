// --- BareMux / Ultraviolet Transport Sync (v2) ---
(function() {
    const sw = self;
    let transportPort = null;
    let transportResolve = null;
    const transportPromise = new Promise(resolve => { transportResolve = resolve; });

    sw.addEventListener('message', (event) => {
        if (event.data && event.data.type === 'baremuxinit' && event.data.port) {
            transportPort = event.data.port;
            // If UV is already initialized, apply port immediately
            if (sw.uv && sw.uv.bareClient && sw.uv.bareClient.worker) {
                sw.uv.bareClient.worker.port = transportPort;
            }
    ...
    // Wait for UVServiceWorker to be defined before monkey-patching
    function hookFetch() {
        if (!sw.UVServiceWorker) {
            setTimeout(hookFetch, 10);
            return;
        }
        ...
        sw.UVServiceWorker.prototype.fetch = async function(event) {
            if (!transportPort) {
                ...
                for (const client of clients) {
                    client.postMessage({ type: 'getPort', port: null });
                }
                await Promise.race([
                    transportPromise,
            ...
            // Ensure the port is attached to the client before fetching
            if (transportPort && this.bareClient && this.bareClient.worker) {
                this.bareClient.worker.port = transportPort;
            }
